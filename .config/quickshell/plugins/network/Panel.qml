...
Popup {
  ...
  Rectangle {
    ...
    color: Network.wifiEnabled ? Theme.alpha(Theme.accent, wifiToggleArea.containsMouse ? 0.28 : 0.2) : Theme.alpha(Theme.fg, wifiToggleArea.containsMouse ? 0.12 : 0.06)
    Row {
      anchors.centerIn: parent
      spacing: 6
      Icon {
        anchors.verticalCenter: parent.verticalCenter
        name: Network.wifiEnabled ? "wifi" : "wifi-off"
        color: Network.wifiEnabled ? Theme.accent : Theme.fg
      }
      Text {
        anchors.verticalCenter: parent.verticalCenter

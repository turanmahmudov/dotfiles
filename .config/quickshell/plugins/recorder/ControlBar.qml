  ...
  Row {
    ...
    Repeater {
      ...
      Rectangle {
        ...
        readonly property bool selected: Recorder.mode === modelData.m
        width: 44
        height: 44
        ...
        MouseArea {
          ...
          onClicked: Recorder.setMode(modeBtn.modelData.m)
        }
      }
    }
    ...
    Rectangle {
      ...
      height: 44
      radius: Style.radiusSmall
      color: Recorder.withAudio ? Theme.alpha(Theme.accent, 0.25)
             : (audioMa.containsMouse ? Theme.alpha(Theme.fg, 0.10) : "transparent")
      Behavior on color { ColorAnimation { duration: 100 } }

      Icon {
        anchors.centerIn: parent
        name: Recorder.withAudio ? "mic" : "mic-off"
        size: 20
        color: Recorder.withAudio ? Theme.accent : Theme.fg
      }
      ...
      MouseArea {
        ...
        cursorShape: Qt.PointingHandCursor
        onEntered: {
          bar.tipText = "Record audio  a"
          bar.tipCenterX = audioChip.mapToItem(bar, audioChip.width / 2, 0).x
        }
        onExited: if (bar.tipText === "Record audio  a") bar.tipText = ""
        onClicked: Recorder.toggleAudio()
      }
    }
  }
